...
class PaymentOrdersStore:
    ...
    def create(self, order_id, user_id, amount, product_name=None, plan=None):
        doc = {"order_id": order_id, "user_id": str(user_id), "amount": float(amount), "product_name": product_name, "plan": plan, "created_at": datetime.now(), "status": "pending"}
        self.collection.update_one({"order_id": order_id}, {"$set": doc}, upsert=True)
        return doc
...
def answer_callback(callback_id):
    ...
    try:
        return requests.post(url, json={"callback_query_id": callback_id}).json()
    except:
        return None
...
class User:
    ...
    def get_data(user_id, key):
        return user_data_store.get(user_id, key)
...
def cmd_start(message, params, options=None):
    ...
    print(f"✅ /start received from {user_id}")  # DEBUG
...
def cmd_autobuy1(message, params, options=None):
    ...
    payment_orders_store.create(order_id=order_id, user_id=user_id, amount=amount, product_name=pt, plan=plan_display)
...
def cmd_verify_payment(message, params, options=None):
    ...
    if order_data.get("status") == "verified":
        send_message(user_id, "✅ Payment already processed.")
        pending_payments.pop(user_id, None)
        pending_payments_store.delete(user_id)
        return True
    
    send_message(user_id, f"{emoji_tag(EMOJIS['clock'], '⏳')} Checking payment...")
...
def cmd_cancel(message, params, options=None):
    ...
    pending_payments_store.delete(user_id)
    User.save_data(user_id, "last_order_id", "")
    send_message(user_id, f"{emoji_tag(EMOJIS['danger'], '❌')} Cancelled")
    return True
...
def cmd_admin(message, params, options=None):
    ...
    if str(user_id) not in [str(a) for a in admins]:
        send_message(user_id, "🚫 You Are Not This Bot Admin")
        return True
...
def cmd_remove_plan(message, params, options=None):
    ...
    bot_data.delete(f"{mod_id}_{day}d_price")
    bot_data.delete(f"{mod_id}_{day}d_reseller_price")
    bot_data.delete(f"{mod_id}_{day}d_keys")
...
def cmd_add_new_plan_process(message, params, options=None):
    ...
    bot_data.save_data(f"{mod_id}_{days}d_price", price)
    bot_data.save_data(f"{mod_id}_{days}d_reseller_price", reseller_price)
    bot_data.save_data(f"{mod_id}_{days}d_keys", [])
...
def cmd_addreseller(message, params, options=None):
    ...
    send_message(user_id, "📩 Send reseller ID:")
...
def cmd_add_reseller_process(message, params, options=None):
    ...
    send_message(user_id, f"✅ User <code>{target}</code> added as reseller")
    pending_commands.pop(user_id, None)
    pending_commands_store.delete(user_id)
    return True
...
def cmd_removereseller(message, params, options=None):
    ...
    send_message(user_id, "📩 Send reseller ID to remove:")
...
def cmd_remove_reseller_process(message, params, options=None):
    ...
    bot_data.save_data("resellers_list", resellers)
    send_message(user_id, f"✅ User <code>{target}</code> removed from resellers")
    pending_commands.pop(user_id, None)
    pending_commands_store.delete(user_id)
    return True
