...
def cmd_shopnawkk(message, params, options=None):
    ...
    for product in products:
        ...
        keyboard.append([{"text": display, "callback_data": f"/shop_{pid}"}])
...
def cmd_edit_product(message, params, options=None):
    ...
    for product in products:
        pid = product.get('product_id')
        keyboard.append([{"text": f"✏️ {product.get('name')}", "callback_data": f"/edit_{pid}"}])
...
def cmd_delete_product(message, params, options=None):
    ...
    for product in products:
        pid = product.get('product_id')
        keyboard.append([{"text": f"🗑️ {product.get('name')}", "callback_data": f"/delete_confirm_{pid}"}])
    keyboard.append([{"text": "🔙 Back", "callback_data": "/admin_products"}])
